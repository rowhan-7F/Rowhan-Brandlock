...
import { Plus, X, Star, SquareDashed } from "lucide-react";
...
export default function FormatTabs({
  ...
  return (
    <div className="relative flex items-center gap-2 pl-6 pr-32 py-3 border-b border-neutral-200 bg-white overflow-x-auto shrink-0">
      ...
      {availableTemplates.length > 0 && (
        ...
        {/* Toggle Safe Zones - tout a droite */}
        {onToggleSafeZones && (
          <button
            type="button"
            onClick={onToggleSafeZones}
            className={[
              "absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold transition-all border-2 shrink-0 bg-white z-20 shadow-sm",
              safeZonesEnabled
                ? "border-[color:var(--bl-primary)] bg-[color:var(--bl-primary)]/5 text-[color:var(--bl-primary)]"
                : "border-neutral-300 bg-white text-neutral-500 hover:border-neutral-400 hover:text-neutral-700",
            ].join(" ")}
            style={{ ["--bl-primary" as any]: primaryColor }}
            title={safeZonesEnabled ? "Masquer les marges de securite" : "Afficher les marges de securite"}
          >
