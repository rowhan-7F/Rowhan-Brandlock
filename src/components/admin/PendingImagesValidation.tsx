...
// ============================================================
//  PENDING IMAGES VALIDATION ÔÇö Inline dans la page validation projet
//  Extrait les images non-valid├®es des slides du projet
//  et permet ├á l'admin de les valider d'un click
// ============================================================
...
export default function PendingImagesValidation({
  ...
  const fetchPendingStatus = useCallback(async () => {
    ...
    try {
      ...
      // R├®cup├¿re les status depuis la DB
      ...
      // Combine : garde uniquement les non-valid├®es
      ...
      imageRefs.forEach((ref) => {
        ...
        if (dbImage && !dbImage.is_approved) {
          // ├ëvite les doublons (une image peut ├¬tre dans plusieurs slides)
  ...
  const handleApproveAll = async () => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession();
      // Sequential pour ├®viter rate limit
  ...
  if (pendingImages.length === 0) {
    return (
      <section className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-2 text-xs text-green-700">
        ...
        Toutes les images de ce projet sont valid├®es
      </section>
    );
  }
    ...
    <section className="bg-orange-50 border-2 border-orange-200 rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-xs font-bold uppercase tracking-widest text-orange-700 flex items-center gap-1.5">
          <ShieldAlert size={12} />
          Images ├á valider ({pendingImages.length})
        </h3>
      ...
      <div className="mt-3 text-[10px] text-orange-700/80">
        ­ƒÆí Ces images ont ├®t├® upload├®es par le studio et n├®cessitent ta validation.
      </div>
    </section>
  );
}
